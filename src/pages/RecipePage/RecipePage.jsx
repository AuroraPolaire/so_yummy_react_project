...
import { selectCurrentRecipe, selectError, selectIsLoading } from 'redux/recipes/recipesSelectors';
...
const RecipePage = props => {
  ...
  const { title, description, time, instructions, ingredients } = currentRecipe;
  return <>
    <h1>{title}</h1>
    ...
    <table>
      ...
      <tbody>
        {ingredients.map(({measure, title, _id, thumb}) => (
          <tr key={_id}>
            <td><img src={thumb} alt={title} /></td>
            <td>{title}</td>
            <td>{measure}</td>
            <td><input type="checkbox" name="shoppingList" /></td>
          </tr>
        ))}
      </tbody>
    </table>
